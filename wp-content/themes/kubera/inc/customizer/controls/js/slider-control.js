wp.customize.controlConstructor['kubera-range-slider'] = wp.customize.Control.extend({
	...
	ready: function() {
		...
		var control = this,
		    ...
		    inputDefault,
		    changeAction,
			controlClass = '.customize-control-kubera-range-slider',
			footerActions = jQuery( '#customize-footer-actions' );
		
		// Set up the sliders
		jQuery( '.kubera-slider' ).each( function() {
		...
		jQuery( controlClass + ' .kubera_range_value input[type=number]' ).on( 'input', function() {
			value = jQuery( this ).attr( 'value' );
			if ( '' == value ) {
				value = -1;
			}
			jQuery( this ).closest( 'label' ).find( '.kubera-slider' ).slider( 'value', parseFloat(value)).change();
		});

		// Handle the reset button
		jQuery( controlClass + ' .kubera-reset' ).on( 'click', function() {
			var icon = jQuery( this ),
				visible_area = icon.closest( '.kubera-range-title-area' ).next( '.kubera-range-slider-areas' ).children( 'label:visible' ),
				input = visible_area.find( 'input[type=number]' ),
				slider_value = visible_area.find( '.kubera-slider' ),
				visual_value = visible_area.find( '.kubera_range_value' ),
		...
		jQuery( controlClass + ' .kubera-range-slider-control' ).each( function() {
			var _this = jQuery( this );
			_this.find( '.kubera-device-controls' ).children( 'span:first-child' ).addClass( 'selected' );
			_this.find( '.range-option-area:first-child' ).show();
		});
		
		// Do stuff when device icons are clicked
		jQuery( controlClass + ' .kubera-device-controls > span' ).on( 'click', function( event ) {
			var device = jQuery( this ).data( 'option' );
			
			jQuery( controlClass + ' .kubera-device-controls span' ).each( function() {
				var _this = jQuery( this );
			...
			jQuery( controlClass + ' .kubera-range-slider-areas label' ).each( function() {
				var _this = jQuery( this );
				if ( device == _this.attr( 'data-option' ) ) {
					_this.show();
					_this.siblings().hide();
