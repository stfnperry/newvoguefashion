( function( api ) {
	...
	api.controlConstructor['kubera-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.kubera-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the kubera_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'kubera_get_all_google_fonts_ajax',
								kubera_customize_nonce: kubera_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.kubera-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.kubera-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.kubera-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.kubera-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.kubera-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.kubera-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.kubera-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.kubera-font-variant' );
			...
			control.container.on( 'change', '.kubera-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.kubera-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.kubera-font-transform select',
				function() {
...
jQuery( document ).ready( function($) {
	...
	jQuery( '.kubera-font-family select' ).selectWoo();
	jQuery( '.kubera-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".kubera-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.kubera-font-variant' ).hide();
		}
	});
