...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Kubera_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 */
	class Kubera_Typography_Customize_Control extends WP_Customize_Control {
		public $type = 'kubera-customizer-typography';

		public function enqueue() {
			wp_enqueue_script( 'kubera-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), KUBERA_VERSION, true );
			wp_enqueue_style( 'kubera-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/selectWoo.min.css', array(), KUBERA_VERSION );

			wp_enqueue_script( 'kubera-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/typography-customizer.js', array( 'customize-controls', 'kubera-typography-selectWoo' ), KUBERA_VERSION, true );
			wp_enqueue_style( 'kubera-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/typography-customizer.css', array(), KUBERA_VERSION );
		}

		public function to_json() {
			parent::to_json();

			$number_of_fonts = apply_filters( 'kubera_number_of_fonts', 200 );
			$this->json[ 'default_fonts_title'] = __( 'System fonts', 'kubera' );
			$this->json[ 'google_fonts_title'] = __( 'Google fonts', 'kubera' );
			$this->json[ 'google_fonts' ] = apply_filters( 'kubera_typography_customize_list', kubera_get_all_google_fonts( $number_of_fonts ) );
			$this->json[ 'default_fonts' ] = kubera_typography_default_fonts();
			$this->json[ 'family_title' ] = esc_html__( 'Font family', 'kubera' );
			$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'kubera' );
			$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'kubera' );
			$this->json[ 'category_title' ] = '';
			$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'kubera' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				#>
				<div id={{{ data.variant.id }}}" class="kubera-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="kubera-font-category">
					<label>
							<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="kubera-hidden-input" />
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="kubera-font-transform">
					<label>
