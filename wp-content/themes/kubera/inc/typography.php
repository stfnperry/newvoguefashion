...
if ( ! function_exists( 'kubera_enqueue_google_fonts' ) ) {
	add_action( 'wp_enqueue_scripts', 'kubera_enqueue_google_fonts', 0 );
	/**
	 * Add Google Fonts to wp_head if needed.
	 *
	 */
	function kubera_enqueue_google_fonts() {
		...
		$not_google = str_replace( ' ', '+', kubera_typography_default_fonts() );
		...
		if ( ! empty( $font_settings ) ) {
			...
			foreach ( $font_settings as $key ) {
				...
				if ( ! isset( $kubera_settings[$key] ) ) {
					continue;
				}

				// If our value is still using the old format, fix it
				if ( strpos( $kubera_settings[$key], ':' ) !== false ) {
					$kubera_settings[$key] = current( explode( ':', $kubera_settings[$key] ) );
				}

				// Replace the spaces in the names with a plus
				$value = str_replace( ' ', '+', $kubera_settings[$key] );

				// Grab the variants using the plain name
				$variants = kubera_get_google_font_variants( $kubera_settings[$key], $key );
		...
		$google_fonts = apply_filters( 'kubera_typography_google_fonts', $google_fonts );

		// Get the subset
		$subset = apply_filters( 'kubera_fonts_subset','' );
		...
		$default_fonts = KUBERA_DEFAULT_FONTS;

		// Create our URL using the arguments
		$fonts_url = add_query_arg( $font_args, '//fonts.googleapis.com/css' );

		// Enqueue our fonts
		wp_enqueue_style('kubera-default-fonts', $default_fonts, array(), null, 'all' );
		if ( $google_fonts ) {
			wp_enqueue_style('kubera-fonts', $fonts_url, array(), null, 'all' );
		}
	}
}

if ( ! function_exists( 'kubera_default_fonts_customize_register' ) ) {
	add_action( 'customize_register', 'kubera_default_fonts_customize_register' );
	/**
	 * Build our Typography options
	 *
	 */
	function kubera_default_fonts_customize_register( $wp_customize ) {
		if ( function_exists( 'kubera_fonts_customize_register' ) ) {
			// Bail if Kubera Premium is active
			return;
		}
		...
		$defaults = kubera_get_default_fonts();

		if ( method_exists( $wp_customize,'register_control_type' ) ) {
			$wp_customize->register_control_type( 'Kubera_Typography_Customize_Control' );
		...
		$wp_customize->add_setting(
			...
			array(
				...
				'sanitize_callback' => 'kubera_sanitize_variants'
			)
		);

		$wp_customize->add_setting(
			'kubera_settings[body_font_weight]',
			array(
		...
		$wp_customize->add_setting(
			'kubera_settings[body_font_transform]',
			array(
		...
		$wp_customize->add_control(
			new Kubera_Typography_Customize_Control(
				$wp_customize,
				'body_typography',
				array(
					'section' => 'font_section',
					'priority' => 1,
					'label' => __( 'Text Font', 'kubera' ),
					'settings' => array(
						'family' => 'kubera_settings[font_body]',
						'variant' => 'font_body_variants',
						'category' => 'font_body_category',
						'weight' => 'kubera_settings[body_font_weight]',
						'transform' => 'kubera_settings[body_font_transform]',
					),
				)
			)
		);
		...
		$wp_customize->add_setting(
			'kubera_settings[body_font_size]',
			array(
		...
		$wp_customize->add_control(
			new Kubera_Range_Slider_Control(
				...
				array(
					...
					'description' => __( 'Font size', 'kubera' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'kubera_settings[body_font_size]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 6,
		...
		$wp_customize->add_setting(
			'kubera_settings[body_line_height]',
			array(
				'default' => $defaults['body_line_height'],
				'type' => 'option',
				'sanitize_callback' => 'kubera_sanitize_decimal_integer',
				'transport' => 'postMessage'
			)
		);
		...
		$wp_customize->add_control(
			new Kubera_Range_Slider_Control(
				...
				array(
					'type' => 'kubera-range-slider',
					'description' => __( 'Line height', 'kubera' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'kubera_settings[body_line_height]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 1,
		...
		$wp_customize->add_setting(
			'kubera_settings[paragraph_margin]',
			array(
		...
		$wp_customize->add_control(
			new Kubera_Range_Slider_Control(
				...
				array(
					'type' => 'kubera-range-slider',
					'description' => __( 'Paragraph margin', 'kubera' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'kubera_settings[paragraph_margin]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 0,
...
if ( ! function_exists( 'kubera_get_all_google_fonts' ) ) {
	/**
	 ...
	 */
	function kubera_get_all_google_fonts( $amount = 'all' ) {
		...
		if ( apply_filters( 'kubera_alphabetize_google_fonts', true ) ) {
			asort( $fonts );
		}

		// Filter to allow us to modify the fonts array
		return apply_filters( 'kubera_google_fonts_array', $fonts );
	}
}

if ( ! function_exists( 'kubera_get_all_google_fonts_ajax' ) ) {
	add_action( 'wp_ajax_kubera_get_all_google_fonts_ajax', 'kubera_get_all_google_fonts_ajax' );
	/**
	 * Return an array of all of our Google Fonts.
	 *
	 *
	 */
	function kubera_get_all_google_fonts_ajax() {
		// Bail if the nonce doesn't check out
		if ( ! isset( $_POST[ 'kubera_customize_nonce' ] ) || ! wp_verify_nonce( sanitize_key( $_POST[ 'kubera_customize_nonce' ] ), 'kubera_customize_nonce' ) ) {
			wp_die();
		}

		// Do another nonce check
		check_ajax_referer( 'kubera_customize_nonce', 'kubera_customize_nonce' );
		...
		$fonts = kubera_get_all_google_fonts();
...
if ( ! function_exists( 'kubera_get_google_font_variants' ) ) {
	/**
	 * Wrapper function to find variants for chosen Google Fonts
	 * Example: kubera_get_google_font_variation( 'Open Sans' )
	 *
	 */
	function kubera_get_google_font_variants( $font, $key = '' ) {
		// Don't need variants if we're using a system font
		if ( in_array( $font, kubera_typography_default_fonts() ) ) {
			return;
		}
		...
		if ( $variants ) {
			...
			return implode(',', apply_filters( 'kubera_typography_variants', $output ) );
		}
	}
}

if ( ! function_exists( 'kubera_get_google_font_category' ) ) {
	/**
	 * Wrapper function to find the category for chosen Google Font
	 * Example: kubera_get_google_font_category( 'Open Sans' )
	 *
	 *
	 *
	 ...
	 */
	function kubera_get_google_font_category( $font, $key = '' ) {
...
if ( ! function_exists( 'kubera_get_font_family_css' ) ) {
	/**
	 ...
	 */
	function kubera_get_font_family_css( $font, $settings, $default ) {
		$kubera_settings = wp_parse_args(
		...
		$no_quotes = array(
			...
			apply_filters( 'kubera_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' )
		);

		// Get our font
		$font_family = $kubera_settings[ $font ];

		if ( 'System Stack' == $font_family ) {
			$font_family = apply_filters( 'kubera_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' );
		}
		...
		if ( in_array( $font_family, $no_quotes ) ) {
			...
			$wrapper_start = '"';
			$wrapper_end = '"' . kubera_get_google_font_category( $font_family, $font );
		}
...
if ( ! function_exists( 'kubera_add_to_font_customizer_list' ) ) {
	add_filter( 'kubera_typography_customize_list', 'kubera_add_to_font_customizer_list' );
	/**
	 ...
	 *
	 */
	function kubera_add_to_font_customizer_list( $fonts ) {
		...
		$all_fonts = kubera_get_all_google_fonts();
		$select_fonts = kubera_get_all_google_fonts( apply_filters( 'kubera_number_of_fonts', 200 ) );

		foreach ( $font_settings as $setting ) {
			// If we don't have a setting, keep going
			if ( ! isset( $kubera_settings[ $setting ] ) ) {
				continue;
			}

			$id = strtolower( str_replace( ' ', '_', $kubera_settings[ $setting ] ) );

			if ( array_key_exists( $id, $select_fonts ) || in_array( $kubera_settings[ $setting ], kubera_typography_default_fonts() ) ) {
				continue;
			}

			$fonts[ strtolower( str_replace( ' ', '_', $kubera_settings[ $setting ] ) ) ] = array(
				'name' => $kubera_settings[ $setting ],
		...
		if ( function_exists( 'kubera_secondary_nav_get_defaults' ) ) {
			...
			if ( ! array_key_exists( $secondary_nav_id, $select_fonts ) && ! in_array( $secondary_nav_settings[ 'font_secondary_navigation' ], kubera_typography_default_fonts() ) ) {
