...
add_action( 'after_setup_theme', 'kubera_setup_woocommerce' );
/**
 * Set up WooCommerce
 *
 */
function kubera_setup_woocommerce() {
	...
	add_action( 'woocommerce_sidebar', 'kubera_construct_sidebars' );
}

if ( ! function_exists( 'kubera_woocommerce_start' ) ) {
	add_action( 'woocommerce_before_main_content', 'kubera_woocommerce_start', 10 );
	/**
	 * Add WooCommerce starting wrappers
	 *
	 */
	function kubera_woocommerce_start() { ?>
...
if ( ! function_exists( 'kubera_woocommerce_end' ) ) {
	add_action( 'woocommerce_after_main_content', 'kubera_woocommerce_end', 10 );
	/**
	 * Add WooCommerce ending wrappers
	 *
	 */
	function kubera_woocommerce_end() { ?>
						</div><!-- .entry-content -->
						<?php
						/**
...
if ( ! function_exists( 'kubera_woocommerce_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'kubera_woocommerce_css', 100 );
	/**
	 * Add WooCommerce CSS
	 *
	 */
	function kubera_woocommerce_css() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return;
		}

		$mobile = apply_filters( 'kubera_mobile_media_query', '(max-width:768px)' );
...
if ( ! function_exists( 'kubera_bbpress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'kubera_bbpress_css', 100 );
	/**
	 * Add bbPress CSS
	 *
	 */
	function kubera_bbpress_css() {
...
if ( ! function_exists( 'kubera_buddypress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'kubera_buddypress_css', 100 );
	/**
	 * Add BuddyPress CSS
	 *
	 */
	function kubera_buddypress_css() {
...
if ( ! function_exists( 'kubera_beaver_builder_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'kubera_beaver_builder_css', 100 );
	/**
	 ...
	 *
	 */
	function kubera_beaver_builder_css() {
