...
if ( ! function_exists( 'kubera_get_setting' ) ) {
	/**
	 * A wrapper function to get our settings.
	 *
	 */
	function kubera_get_setting( $setting ) {
		$kubera_settings = wp_parse_args(
			get_option( 'kubera_settings', array() ),
			kubera_get_defaults()
		);

		return $kubera_settings[ $setting ];
	}
}

if ( ! function_exists( 'kubera_get_layout' ) ) {
	/**
	...
	function kubera_get_layout() {
		...
		$layout = $kubera_settings['layout_setting'];

		// Get the individual page/post sidebar metabox value
		$layout_meta = ( isset( $post ) ) ? get_post_meta( $post->ID, '_kubera-sidebar-layout-meta', true ) : '';
		...
		if ( is_single() && ! $buddypress ) {
			$layout = null;
			$layout = $kubera_settings['single_layout_setting'];
		}
		...
		if ( is_home() || is_archive() || is_search() || is_tax() ) {
			$layout = null;
			$layout = $kubera_settings['blog_layout_setting'];
		}

		// Finally, return the layout
		return apply_filters( 'kubera_sidebar_layout', $layout );
	}
}

if ( ! function_exists( 'kubera_get_footer_widgets' ) ) {
	/**
	 ...
	 */
	function kubera_get_footer_widgets() {
		// Get current post
		global $post;
		...
		$widgets = $kubera_settings['footer_widget_setting'];

		// Get the individual footer widget metabox value
		$widgets_meta = ( isset( $post ) ) ? get_post_meta( $post->ID, '_kubera-footer-widget-meta', true ) : '';
		...
		// Finally, return the layout
		return apply_filters( 'kubera_footer_widgets', $widgets );
	}
}

if ( ! function_exists( 'kubera_show_excerpt' ) ) {
	/**
	 * Figure out if we should show the blog excerpts or full posts
	 *
	 */
	function kubera_show_excerpt() {
		// Get current post
		global $post;
		...
		$more_tag = apply_filters( 'kubera_more_tag', strpos( $post->post_content, '<!--more-->' ) );

		// Check the post format
		$format = ( false !== get_post_format() ) ? get_post_format() : 'standard';

		// Get the excerpt setting from the Customizer
		$show_excerpt = ( 'excerpt' == $kubera_settings['post_content'] ) ? true : false;
		...
		// Return our value
		return apply_filters( 'kubera_show_excerpt', $show_excerpt );
	}
}

if ( ! function_exists( 'kubera_show_title' ) ) {
	/**
	 ...
	 */
	function kubera_show_title() {
		return apply_filters( 'kubera_show_title', true );
	}
}

if ( ! function_exists( 'kubera_padding_css' ) ) {
	/**
	 ...
	 */
	function kubera_padding_css( $top, $right, $bottom, $left ) {
...
if ( ! function_exists( 'kubera_get_link_url' ) ) {
	/**
	 * Return the post URL.
	 *
...
if ( ! function_exists( 'kubera_get_navigation_location' ) ) {
	/**
	 ...
	 */
	function kubera_get_navigation_location() {
		return apply_filters( 'kubera_navigation_location', kubera_get_setting( 'nav_position_setting' ) );
	}
}
