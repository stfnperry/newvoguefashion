...
namespace SHWPortfolioCatalog\Helpers;

class View
{
    public static function locate($view, $path = '', $defaultPath = ''){
        if (!$path) {
            $path = \SHWPortfolioCatalog()->viewPath();
        }
        if (!$defaultPath) {
            $defaultPath = \SHWPortfolioCatalog()->pluginPath() . 'resources/views/';
        }
    ...
    public static function renderOnce($view, $args = array(), $path = '', $defaultPath = '')  {
        if ($args && is_array($args)) {
            extract($args);
        }
        $located = self::locate($view, $path, $defaultPath);
